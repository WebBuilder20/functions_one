...
class YoutubeTrans extends StatefulWidget {
  const YoutubeTrans({Key? key}) : super(key: key);
...
class _YoutubeTransState extends State<YoutubeTrans> {
  ...
  String convertSC() {
    ...
    if (autoPlay && loop) {
      sc = sc.replaceFirst(_youtubeID!, '?version=3&autoplay=1&loop=1&playlist='+_youtubeID);
    }

    if (autoPlay && !loop) {
      sc = sc.replaceFirst(_youtubeID!, '?version=3&autoplay=1&playlist='+_youtubeID);
    }

    if (loop && !autoPlay) {
      sc = sc.replaceFirst(_youtubeID!, '?version=3&loop=1&playlist='+_youtubeID);
    }

    if (_selectedValue == "가로 길이에 맞춤") {
      int _wantedHeight = resizeCal(_oriWidth, _oriHeight, int.parse(_widthValueController.text), true);
    ...
    if (_selectedValue == "세로 길이에 맞춤") {
      int _wantedWidth = resizeCal(_oriWidth, _oriHeight, int.parse(_heightValueController.text), false);
                    ...
                    child: Padding(
                      ...
                      child: TextFormField(
                        ...
                        validator: (value) {
                          ...
                          } else if (_regID.allMatches(_sourceCodeController.text).length > 1) {
                            return '다수의 소스코드를 한번에 변환할 수 없습니다.';
                          } else if (_sourceCodeController.text.trim().split("width").length != 2
                              || _sourceCodeController.text.trim().split("height").length != 2
                              || _sourceCodeController.text.trim().split("embed/").length != 2
                              || (_sourceCodeController.text.trim().split('"').length + _sourceCodeController.text.trim().split("'").length) < 10
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              SizedBox(width: 8),
                              Text("자동 재생", style: TextStyle(fontSize: 15)),
                            ],
                          ),
                          SizedBox(width: 80),
                          Row(
                                      ...
                                      return DropdownMenuItem(
                                        ...
                                        child: Padding(
                                          padding: EdgeInsets.only(bottom:2.8),
                                            child: Text(value, style: TextStyle(color: Colors.black87, fontSize: 16.2))),
                                      );
                                    }).toList(),
                                    onChanged: (value) {
                                      setState(() {
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              } else if (_selectedValue == "가로 길이에 맞춤" && _widthValueController.text.isEmpty) {
                                alertBox(context, "변환 실패", "변환할 영상의 가로 길이가 비어있습니다.");
                              } else if (_selectedValue == "세로 길이에 맞춤" && _heightValueController.text.isEmpty) {
                                alertBox(context, "변환 실패", "변환할 영상의 세로 길이가 비어있습니다.");
                              } else if (_selectedValue == "사용자 맞춤 설정" && (_widthValueController.text.isEmpty || _heightValueController.text.isEmpty)) {
